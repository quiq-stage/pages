(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      '--nesp-card-bg:#ecebea;',
      '--nesp-group-bg:#f4f3f2;',
      '--nesp-text:#262626;',
      '--nesp-muted:#6f6f6f;',
      '--nesp-border:#dfdcd8;',
      '--nesp-hover:#efedeb;',
      '--nesp-shadow:0 14px 38px rgba(0,0,0,.22);',
      '--nesp-shadow-btn:0 10px 28px rgba(0,0,0,.20);',
      '--nesp-radius-card:18px;',
      '--nesp-radius-group:16px;',
      '--nesp-radius-row:14px;',
      '}',

      '#' + CONFIG.ids.root + '{font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:var(--nesp-text);}',
      '.nesp-help-sr-only{position:absolute !important;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0;}',

      '#' + CONFIG.ids.backdrop + '{position:fixed;inset:0;background:rgba(0,0,0,.28);opacity:0;pointer-events:none;transition:opacity 220ms var(--nesp-ease);z-index:calc(var(--nesp-z) - 1);}',
      ...
      '}',
      '#' + CONFIG.ids.panel + '.is-open{opacity:1;pointer-events:auto;transform:translateY(0) scale(1);}',

      '.nesp-help-header{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:14px;}',
      '.nesp-help-title{margin:0;font-size:18px;line-height:1.2;font-weight:700;color:var(--nesp-text);}',
      '#' + CONFIG.ids.close + '{width:34px;height:34px;min-width:34px;border:0;border-radius:999px;background:#f2f1f0;color:#2f2f2f;display:grid;place-items:center;cursor:pointer;font-size:22px;line-height:1;padding:0;transition:background 140ms ease, transform 120ms ease;}',
      '#' + CONFIG.ids.close + ':hover{background:#ebe8e5;transform:scale(1.03);}',

      '.nesp-help-section{margin-top:8px;}',
      '.nesp-help-section-label{margin:0 0 8px 2px;font-size:12px;line-height:1.3;font-weight:500;color:var(--nesp-muted);}',
      '.nesp-help-group{background:var(--nesp-group-bg);border-radius:var(--nesp-radius-group);padding:8px;}',
      '.nesp-help-list{display:grid;gap:4px;}',

      '.nesp-help-row{',
      'display:flex;align-items:flex-start;gap:12px;width:100%;min-height:52px;padding:12px 12px;border:0;background:transparent;border-radius:var(--nesp-radius-row);',
      'text-decoration:none;color:var(--nesp-text);box-sizing:border-box;transition:background 140ms ease, transform 120ms ease;',
      '}',
      '.nesp-help-row:hover{background:var(--nesp-hover);transform:translateY(-1px);}',
      '.nesp-help-row:active{transform:translateY(0);}',

      '.nesp-help-icon-wrap{width:24px;min-width:24px;height:24px;display:grid;place-items:center;color:#1f1f1f;margin-top:2px;}',
      '.nesp-help-icon-wrap svg{width:20px;height:20px;display:block;}',

      '.nesp-help-copy{min-width:0;flex:1 1 auto;}',
      '.nesp-help-row-title{display:block;font-size:15px;line-height:1.3;font-weight:500;color:var(--nesp-text);}',
      '.nesp-help-row-meta{display:block;margin-top:4px;font-size:12px;line-height:1.45;color:var(--nesp-muted);}',

      '.nesp-help-chevron{width:18px;min-width:18px;display:grid;place-items:center;color:#777;font-size:24px;line-height:1;margin-top:1px;}',

      '.nesp-help-chat-row{cursor:pointer;}',
      '.nesp-help-chat-row .nesp-help-row-title{font-weight:500;}',
      '.nesp-help-chat-row .nesp-help-row-meta,.nesp-help-phone-row .nesp-help-row-meta{max-width:245px;}',

      '#' + CONFIG.ids.launcher + '{position:fixed;right:var(--nesp-right);bottom:var(--nesp-bottom);z-index:var(--nesp-z);background:transparent;border:0;padding:0;margin:0;cursor:pointer;}',
      '#' + CONFIG.ids.pill + '{height:52px;width:52px;border-radius:999px;background:var(--nesp-launcher-bg);border:1.5px solid var(--nesp-launcher-border);box-shadow:var(--nesp-shadow-btn);display:flex;align-items:center;justify-content:flex-start;padding:0;gap:0;overflow:hidden;user-select:none;transition:width 240ms var(--nesp-ease), padding 240ms var(--nesp-ease), gap 240ms var(--nesp-ease), box-shadow 180ms ease;}',
      '.nesp-help-launcher-icon{width:52px;height:52px;min-width:52px;display:grid;place-items:center;color:var(--nesp-launcher-text);line-height:0;}',
      '.nesp-help-launcher-icon svg{width:24px;height:24px;display:block;}',
      '.nesp-help-launcher-label{font-size:14px;font-weight:600;color:var(--nesp-launcher-text);white-space:nowrap;opacity:0;transition:opacity 200ms ease;}',

      '@media (hover:hover) and (pointer:fine){',
      '#' + CONFIG.ids.launcher + ':hover #' + CONFIG.ids.pill + '{width:var(--nesp-pill-open-width);padding:0 var(--nesp-pill-pad-right) 0 var(--nesp-pill-pad-left);gap:var(--nesp-pill-gap);}',
      '#' + CONFIG.ids.launcher + ':hover .nesp-help-launcher-label{opacity:1;}',
      '}',

      '#' + CONFIG.ids.launcher + '.is-open #' + CONFIG.ids.pill + '{width:var(--nesp-pill-open-width);padding:0 var(--nesp-pill-pad-right) 0 var(--nesp-pill-pad-left);gap:var(--nesp-pill-gap);}',
      '#' + CONFIG.ids.launcher + '.is-open .nesp-help-launcher-label{opacity:1;}',

      '#' + CONFIG.ids.launcher + ':focus-visible, #' + CONFIG.ids.close + ':focus-visible, .nesp-help-row:focus-visible{outline:2px solid #4d90fe;outline-offset:2px;}',

      '@media (max-width:640px){',
      '#' + CONFIG.ids.panel + '{right:12px;bottom:86px;width:calc(100vw - 24px);padding:16px 14px 14px;}',
